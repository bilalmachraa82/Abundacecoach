...
import {
  ...
  calculateExpenseManagementScore,
...
export function calculateFinancialHealthScore(transactions: Transaction[]) {
  ...
  const savingsRate = lastMonth.income > 0 ? (lastMonth.savings / lastMonth.income) * 100 : 0;
  ...
  const totalScore = Math.round(breakdown.reduce((sum, item) => sum + item.score, 0));
